...
for fold in range(nfolds):
    ...
    """
    gmr_movie = np.array([np.mean(train[train[:,1]==(i+1)][:,2]) for i in range(np.max(ratings[:,1]))])
    ...
    err_train[fold]=np.sqrt(np.mean((np.array([(train[j,2] - gmr_movie[train[j,1]-1]) for j in range(len(train[:,2]))]))**2))
    err_train_MAE[fold]=np.mean(np.absolute(np.array([(train[j,2] - gmr_movie[train[j,1]-1]) for j in range(len(train[:,2]))])))

    #apply the model to the test set:
    err_test[fold]=np.sqrt(np.mean((np.array([(test[j,2] - gmr_movie[test[j,1]-1]) for j in range(len(test[:,2]))]))**2))
    err_test_MAE[fold]=np.mean(np.absolute(np.array([(test[j,2] - gmr_movie[test[j,1]-1]) for j in range(len(test[:,2]))])))
